...
class UserController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        return view('member.system.user.index', compact('lists'));
    }
    ...
    public function create(Request $request)
    {
        try {
            ...
            if ($request->isMethod('POST')) {
                ...
                if ($validator->fails()) {
                    echo "效验失败";
                    return redirect('/member/system/user/create')
                        ->withInput()
                ...
                $user->save();
                if ($user) {
                    return redirect('/member/system/user')->withSuccess('保存成功！');
                }
                return Redirect::back()->withErrors('保存失败！');
            }
            $systems = Distribution::all();
            return view('member.system.user.create', compact('user', 'systems'));
