    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($request->isMethod('POST')) {
                ...
                if ($validator->fails()) {
                    return redirect('/member/record/signature/create/')
                        ->withInput()
                ...
                $signature->save();
                if ($signature) {
                    return redirect('/member/record/detail')->withSuccess('保存成功！');
                }
            ...
            return view('member.record.resource.create', compact('signature', 'enterprises'));
    ...
    public function edit($id, Request $request)
    {
        try {
            ...
            if ($request->isMethod('POST')) {
                ...
                $signature->save();
                if ($signature) {
                    return redirect('/member/record/signature')->withSuccess('保存成功！');
                }
                return Redirect::back()->withErrors('保存失败！');
            }

            return view('member.record.resource.edit', compact('signature'));
